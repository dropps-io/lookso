...
export async function updateRegistry(sender: string, postHash: string, jsonUrl: string, web3: Web3) {
    const postValidator = new web3.eth.Contract(PostValidator.abi as AbiItem[], POST_VALIDATOR_ADDRESS);
    await postValidator.methods.post(postHash, jsonUrl).send({from: sender});
}