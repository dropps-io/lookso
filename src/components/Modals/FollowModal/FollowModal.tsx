...
import { type ProfileFollowingDisplay } from '../../../models/profile';
...
const FollowModal: FC<FollowModalProps> = props => {
  ...
  useEffect(() => {
    ...
    if (props.type === 'followers')
      fetch = fetchProfileFollowers(props.address, 0, account || undefined);
    else fetch = fetchProfileFollowing(props.address, page, account || undefined);
  ...
  function handleScroll() {
    if (ref.current != null && !loading && !fullyLoaded) {
  ...
  async function followUser(address: string) {
    ...
    setProfiles(existing =>
      existing.map(p => {
        if (p.address === address) p.following = true;
        return p;
      })
    );
    try {
      ...
      setProfiles(existing =>
        existing.map(p => {
          if (p.address === address) p.following = false;
          return p;
        })
      );
                ...
                index === profiles.length - 25 ? (
                  <div ref={ref} key={index} className={`${styles.Profile}`}>
                    <div className={styles.ProfileDisplay}>
                      <div
                        onClick={() => {
                          goToProfile(profile.address);
                        }}
                        className={styles.ProfileImgMedium}
                        style={{
                          backgroundImage: `url(${
                            profile.image ? formatUrl(profile.image) : DEFAULT_PROFILE_IMAGE
                          })`,
                        }}
                      />
                      <span
                    ...
                    {profile.following ? (
                      <button
                        onClick={async () => {
                          await unfollowUser(profile.address);
                        }}
