...
const UserTag: FC<UserTagProps> = props => (
  <span
    ...
    className={`${props.colorReversed ? styles.UserTagReverse : styles.UserTag} ${
      props.onClick != null ? styles.Pointer : ''
    }`}
    title={
      props.username.length > 12
        ? (props.username ? '@' + props.username : 'unnamed') +
          '#' +
          (props.address ? props.address.slice(2, 6).toUpperCase() : '')
        : undefined
    }
  >
    {props.username ? (
      <span className={`${styles.Username} ${props.postHierarchy}`}>
