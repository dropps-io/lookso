...
const AddressFeedDisplay: FC<AddressFeedDisplayProps> = props => {
  if (props.standard === 'LSP0') {
    return (
      <span
        onClick={e => {
          props.onClick(e, props.address);
        }}
        className={`${styles.AddressFeedDisplay} ${styles.Profile} ${props.postHierarchy}`}
        data-testid="AddressFeedDisplay"
      >
        <UserTag
          username={props.name}
    ...
    return (
      <span
        ...
        className={`${styles.AddressFeedDisplay} ${styles.KeyManager} ${props.postHierarchy}`}
        data-testid="AddressFeedDisplay"
      >
    ...
    props.standard === 'LSP7' ||
    props.standard === 'LSP8' ||
    props.standard === 'ERC20' ||
    props.standard === 'ERC721' ||
    props.standard === 'ERC777' ||
    props.standard === 'ERC1155'
  ) {
    return (
      <span
        onClick={e => {
        ...
        className={`${styles.AddressFeedDisplay} ${styles.Asset}`}
        data-testid="AddressFeedDisplay"
      >
    ...
    return (
      <span
        ...
        className={`${styles.AddressFeedDisplay} ${styles.UnknownStandard} ${props.postHierarchy}`}
        data-testid="AddressFeedDisplay"
      >
        {shortenAddress(props.address, 3)}
      </span>
