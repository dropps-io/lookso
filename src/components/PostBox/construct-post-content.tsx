...
const ParamContent: FC<ParamContentProps> = props => {
  ...
  if (props.param.type === 'address') {
    return (
      <strong title={props.param.value}>
        <AddressFeedDisplay
          ...
          standard={
            props.param.additionalProperties.interfaceCode
              ? props.param.additionalProperties.interfaceCode
              : ''
          }
    ...
    return (
      <>
        <strong
          className={`${styles.Bytes32Parameter} ${props.postHierarchy}`}
          title={props.param.value}
          onClick={e => {
            copyToClipboard(e, props.param.value);
          }}
        >
          {shortenAddress(props.param.value, 5)}
        </strong>
        <span className={`copied ${copied ? 'copied-active' : ''} ${props.postHierarchy}`}>
          Copied to clipboard
        </span>
      </>
    );
        ...
        {props.param.display.split(';').map(permission => (
          <>
            <span
              ...
              key={permission}
              style={{
                backgroundColor: `#${keccak256(permission ? permission : 'default').slice(4, 8)}`,
              }}
      ...
      {props.text.split('\n').map((sentence, index) => (
        <span key={index}>
          ...
          {sentence.split(' ').map((word, i) =>
            word.match(
              /(< href=")?((https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)))(">(.*)<\/a>)?/gi
            ) != null ? (
              <a
                key={i}
                className={'Link'}
                ...
                rel="noreferrer"
              >{`${word} `}</a>
            ) : word.match(USER_TAG_REGEX) != null ? (
              <span key={i}>
                <span
                  onClick={async () => {
                    await goToProfile(word);
                  }}
                  className={`${styles.ProfileTagged} ${props.postHierarchy}`}
                >
                  {word.split('#')[0]}
                  <span className={`${styles.Digits} ${props.postHierarchy}`}>
                    #{word.split('#')[1]}
                  </span>
                </span>
                {` `}
