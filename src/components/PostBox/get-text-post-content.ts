...
export function getTextPostContent(text: string, params: {[key: string]: FeedDisplayParam}) {
  return text.split(/{([^}]+)}/).map((entry) => params[entry] ? paramContent(params[entry]) : entry).join(' ');
}
...
function paramContent(param: FeedDisplayParam) {
  if (param.type === 'address') {
    if (param.additionalProperties.interfaceCode) {
      ...
      else if (code === 'LSP7' || code === 'LSP8' || code === 'ERC20' || code === 'ERC721' || code === 'ERC777' || code === 'ERC1155') {
